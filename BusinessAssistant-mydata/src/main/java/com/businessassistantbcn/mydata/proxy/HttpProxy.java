package com.businessassistantbcn.mydata.proxy;
...
public class HttpProxy {
    ...
    public HttpProxy(PropertiesConfig config){
        ...
        httpClient = HttpClient.create()
                ...
                .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, 30000)//TODO - externalizar a properties
                //.responseTimeout(Duration.ofMillis(config.getConnection_timeout()))
                .responseTimeout(Duration.ofMillis(30000))//TODO
                .doOnConnected(conn ->
                        //conn.addHandlerLast(new ReadTimeoutHandler(config.getConnection_timeout(), TimeUnit.MILLISECONDS))//TODO
                        conn.addHandlerLast(new ReadTimeoutHandler(30000, TimeUnit.MILLISECONDS))
                                //.addHandlerLast(new WriteTimeoutHandler(config.getConnection_timeout(), TimeUnit.MILLISECONDS)));//TODO
    ...
    private void acceptedCodecs(ClientCodecConfigurer clientCodecConfigurer) {
        ...
        //clientCodecConfigurer.defaultCodecs().maxInMemorySize(config.getMaxBytesInMemory());//TODO
