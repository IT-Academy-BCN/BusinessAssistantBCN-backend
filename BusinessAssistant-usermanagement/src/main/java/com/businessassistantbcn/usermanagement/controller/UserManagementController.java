...
public class UserManagementController {
    ...
    public ResponseEntity<User> getUser(@PathVariable ("uuid") String uuid) {
    	Optional<User> findingUser = userDAO.findByuuid(uuid);
    ...
    public ResponseEntity<User> newUser(@RequestBody User user){
    	Optional<User> findingUser = userDAO.findByEmail(user.getEmail());
    	if (findingUser.isPresent()) {
    		System.out.println("Usuario ya existente");
    		return ResponseEntity.badRequest().build();
    	} else {
    		User newUser = findingUser.get();
    		return ResponseEntity.ok(userDAO.insert(newUser));
    	}
    }
    ...
    public ResponseEntity<User> updateUser(@PathVariable("uuid") String uuid, @RequestBody User user){
    	...
    	if (findingUser.isPresent()) {
    		...
    		return ResponseEntity.ok(userDAO.insert(updateableUser));
    	} else {
    ...
    public ResponseEntity<User> deleteUser(@PathVariable("uuid") String uuid){
    	...
    	if (findingUser.isPresent()) {
    		...
    		System.out.println("Estás seguro que desea borrar el usuario " + uuid + "? (Aprete y para aceptar)");
    		String resp = escaner.next(); 
    		escaner.close();
    		if (resp == "y") {
    			User deletableUser = findingUser.get();
    			userDAO.delete(deletableUser);
