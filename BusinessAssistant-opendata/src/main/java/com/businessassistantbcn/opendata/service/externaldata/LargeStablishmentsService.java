		...
		try {
			...
			return response.flatMap(dto ->{
				try {
					...
					log.error("Error con el filtrado y la paginación por distrito en largeStablishments" +  e.getMessage());
		...
		try {
			...
			return response.flatMap(dto ->{
				try {
					...
					log.error("Error con la paginación en largeStablishments" +  e.getMessage());
					throw new ResponseStatusException(HttpStatus.BAD_REQUEST, e.getMessage(), e);
				}
			} );
		} catch (MalformedURLException e) {
