...
public class Communication {
    ...
    public List<GranEstabliment> getAllGranEstabliments(){
        ...
        ResponseEntity<List<GranEstabliment>> responseEntity =
                restTemplate.exchange(URL, HttpMethod.GET, null, new ParameterizedTypeReference<List<GranEstabliment>>() {}); // RestTemplate object is used to realize HTTP requests. The 3d parameter of the exchange() method is a ResponseEntity, which is used if you want to add a body to the HTTP request. But in this case we use HttpMethod.GET, and the body of the GET method is always empty. That's why we use null as a 3d parameter. The 4th parameter is ParameterizedTypeReference<>() which is a class helper, the goal of which is to transmit a generic type. In our case the generic type is a list of GranEstabliment. So, using this whole line we send a request, and we get its result in the ResponseEntity
        ...
        List<GranEstabliment> AllGranEstabliments = responseEntity.getBody();

        return AllGranEstabliments;
    }


    // This method will get a concrete gran establiment by id from the opendata REST SERVICE. Here we use HTTP request, the result of which is getting a GranEstabliment
    public GranEstabliment getGranEstabliment(long id){

        // This operation we accomplish using the RestTemplate class helper as well. For this purpose I use the getForObject() method. The 1st parameter is our URL + id of the object. And the 2d parameter is what we expect to get as a response to this HTTP request. As we expect to get a GranEstabliment, so we use Reflection of the GranEstabliment.class
        GranEstabliment granEstabliment = restTemplate.getForObject(URL + "/" + id, GranEstabliment.class);
